...
if ($LASTEXITCODE -eq 0) #if build ok
{
    ...
    docker build -t eirikraywiz/openremote:$($tag) ./manager/build/install/manager # bygg docker imaget med navn openremote og tag custom FRA mappen "gradle installDist" legger builden.
    }
    else {
